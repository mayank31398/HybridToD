        ...
        if belief is not None:
            for i, (domain, (num_results, results)) in enumerate(database_results.items()):
                if domain not in belief:
                    continue
                elif "ruk" in belief[domain].keys():
                    belief[domain].pop("ruk")
                elif "topic" in belief[domain].keys():
                    belief[domain].pop("topic")
        ...
        for domain, domain_bs in belief.items():
            ...
            if self.domain_not_empty(domain_bs) or \
                ...
                def query_single(domain_bs):
                    blocked_slots = {'people', 'booked', 'stay', 'ruk', 'topic'}
...
class BeliefStateTransformation:
    ...
    def __call__(self, belief_state, dialogue_act, active_domain):
        ...
        for domain, domain_bs in belief_state.items():
            ...
            '''
            if 'ruk' in domain_bs:  # extend belief state in TAHOE
                new_domain_bs.update({"ruk": domain_bs['ruk']})
                new_domain_bs.update({"topic": domain_bs['topic']})
            '''
        ...
        items = [(active_domain, active_bs)] if active_bs is not None else []  # put active domain at first
...
def delexicalise_spans(response, spans, allowed_slots=None, verbose=False):
...
def MyDelexicaliseSpans(response, spans, database_results, fuzzy_t = 60, verbose=False):
    def Skippable(key, value, database_results):
        if (key == "reference"):
            return False
        for domain in database_results:
            for ent in database_results[domain][1]:
                for k, v in ent.items():
                    if (k == key):
                        if (v.strip().lower() == value.strip().lower() or
                                fuzz.partial_ratio(value.strip().lower(), v.strip().lower()) >= fuzzy_t
                                or value.strip().lower() in value.strip().lower()):
                            return False
        return True
    ...
    for i, span in enumerate(spans):
        if span[1] == 'Fee' and 'vary' in span[-3]:
            pass
        elif Skippable(DELEX_LABEL_MAP[span[1]], span[2], database_results):
            pass
        else:
            new_spans.append(span)
    spans = new_spans
...
def delexicalise(utt, return_replacements=False, database_results=None, belief=None, spans=None, verbose=False):
    database_results = Lexicalizer.extend_database_results(database_results, belief)
    ...
    utt, replacements = delexicalise_spans(utt, spans, allowed_keys, verbose=verbose)
...
def MyDelexicalise(utt, return_replacements=False, database_results=None, belief=None, spans=None, verbose=False):
    database_results = Lexicalizer.extend_database_results(database_results, belief)

    # First we use the span_info annotations
    spans = sorted(spans, key=lambda x: x[-2])
    utt, replacements = MyDelexicaliseSpans(utt, spans, database_results, verbose=verbose)
    if return_replacements:
        replacements = [x[:2] for x in replacements]
        # replacements.sort(key=lambda k: k[0])
        return utt, replacements
...
def export_data(path, zipf):
    ...
    for split in ['train', 'val', 'test']:
        ...
        with zipfile.ZipFile(BytesIO(zipf.read(f'{split}.json.zip'))) as zsplitf:
        ...
        for sess_id, sess in data.items():
            ...
            for i, turn in enumerate(sess['log']):
                ...
                if item['speaker'] == 'system':
                    ...
                    # Detect active domain and Judge turn type
                    ruk = False
                    for domain, meta in belief.items():
                        if "ruk" in meta.keys():
                            item['active_domain'] = domain
                            ruk = True
                            item['document'] = turn['knowledge']['snippet']['body']
                            belief[domain]['topic'] = " ".join(turn['knowledge']['keywords'])
                            break
                    item['uk_based'] = ruk
                    if not ruk:
                        item['document'] = ""
                    ...
                    '''In TAHOE, this needs modification to get extended belief'''
                    belief = transform_belief(belief, da, active_domain)
                    belief = fix_belief_from_database(belief)  # normalize value (entity name)
                    item['belief'] = belief
                    if 'bus' in belief:
    ...
    for split, dialogues in splits:
        for dialogue in tqdm(dialogues, desc=f'delexicalising {split}'):
            for item in dialogue['items']:
                ...
                if item['speaker'] == 'system':
                    ...
                    '''
                    if item['uk_based']:  # do not delex newly inserted turns
                        delexicalised_text = normalize(text)
                    '''
                    delexicalised_text = delexicalise(text, return_replacements=False,
                                                      database_results=database_results,
                                                      belief=deepcopy(belief), spans=span_info)
        ...
        with open(os.path.join(path, f'{split}.json'), 'w+') as f:
            json.dump(dict(dialogues=dialogues, domains=MW_DOMAINS), f, indent=4)
...
def preprocess():
    ...
    with zipfile.ZipFile(os.path.join(path, 'data_aug.zip')) as zipf:
        export_data(path, zipf)
