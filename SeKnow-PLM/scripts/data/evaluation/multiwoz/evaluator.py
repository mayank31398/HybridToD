...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def evaluate(self, beliefs, databases, documents, responses, progressbar=False, pre_num=5):
        ...
        for idx, (items, goal, beliefs, databases, documents, responses, booked_domains) in enumerate(dialogues):
            for turn_idx, (item, belief, database, document) in enumerate(zip(items, beliefs, databases, documents)):
                ...
                if item.uk_based:
                    if uk_based_pre:
                        detect_tp += 1
                        sum_mrr5 += self.reciprocal_rank(gold_doc, document, pre_num=pre_num)
                        sum_r1 += self.recall_at_k(gold_doc, document, 1)
                        sum_r5 += self.recall_at_k(gold_doc, document, pre_num)
                    else:
                        detect_fn += 1
                else:
                    if uk_based_pre:
                    ...
                    if ('thank' not in user_utter) and ('bye' not in user_utter) and (gold_belief_str != ""):
                        bf_total += 1
