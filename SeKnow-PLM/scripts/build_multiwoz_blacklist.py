...
def enumerate_multiwoz_invalid_indices(dataset_name, logger, correct_requestables=False):
    ...
    databases = (item.database for item in dataset)
    documents = (item.document for item in dataset)
    dialogues = evaluator.pack_dialogues(dataset, beliefs, databases, documents, responses)
    ...
    with tqdm(total=len(dataset), desc='identifying bad dialogues') as progress:
        for idx, (items, goal, beliefs, databases, documents, responses, booked_domains) in enumerate(dialogues):
            ...
            provided_requestables, venue_offered = evaluator._get_requestables_and_venues(
                items, beliefs, responses, booked_domains)
