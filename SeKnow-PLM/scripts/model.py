...
EOD_TK = '<|eod|>'
EOT_TK = '<|endoftext|>'
SPECIAL_TOKENS = [EOB_TK, EOKB_TK, EOD_TK]
...
class AuGPTModel(transformers.GPT2PreTrainedModel):
    ...
    def forward(self,
        ...
        hidden_states = transformer_outputs[0]  # (bsz, XX, T, hidden_size)
        ...
        def gather_auxiliary_features(token_ids):
            if token_ids is None:
                ...
                token_ids = token_ids.unsqueeze(-1).unsqueeze(-1)  # (bsz, XX) -> (bsz, XX, 1, 1)
                # (bsz, XX, 1, 1) -> (bsz, XX, 1, hidden_size)
            ...
            # (bsz, XX, T, hidden_size) -> (bsz, XX, hidden_size)
            logits = hidden_states.gather(-2, token_ids).squeeze(-2)  # extract the hidden_states at token_ids
        ...
        belief_loss, response_loss, response_ce = None, None, None
        if belief_labels is not None:
            ...
            if self.config.response_loss == 'ce':
                ...
                response_loss, response_ce = candidate_ce_fct(shift_logits, shift_response_labels, return_ce=True)
            else:
...
class ModelPredictor:
    ...
    def predict_belief(self, contexts):
        ...
        for ctx in contexts:
            ...
            offset = len(sample[0])  # length of context
    ...
    def predict_response(self, contexts, beliefs, dbs, docs):
        insert_labels = data.utils.InsertLabelsTransformation()
        tokenize = data.utils.TokenizerTransformation(
            self.tokenizer,
        ...
        responses = []
        # TODO: batch generation
        for context, belief, db, doc in zip(contexts, beliefs, dbs, docs):
            sample = insert_labels((context, belief, db, None, 1, None, None, doc))
            sample = tokenize.get_tokens(sample)[0]
            sample = torch.tensor(sample, dtype=torch.int64).to(self.device)
            sample = sample.view(1, *sample.shape)  # (1, T)
            greedy_output = self.model.generate(
