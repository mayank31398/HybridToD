...
class AutoDocbase:
    ...
    def load(pretrained_model_name_or_path: str, **kwargs):
        ...
        if os.path.isdir(pretrained_model_name_or_path):
            docbase_file = os.path.join(pretrained_model_name_or_path, 'document_base.json')
        elif os.path.isfile(pretrained_model_name_or_path) or \
                transformers.file_utils.is_remote_url(pretrained_model_name_or_path):
            docbase_file = pretrained_model_name_or_path
        else:
            docbase_file = transformers.file_utils.hf_bucket_url(
                pretrained_model_name_or_path, filename='document_base.json'
            )
        ...
        try:
            ...
            with open(resolved_database_file, "r") as f:
                document_base = json.load(f)
            docbase = Documentbase(document_base)

        except EnvironmentError:
            msg = (
                f"Can't load config for '{pretrained_model_name_or_path}'. Make sure that:\n\n"
                f"- '{pretrained_model_name_or_path}' is a correct model identifier listed on 'https://huggingface.co/models'\n\n"  # noqa: E501
                f"- or '{pretrained_model_name_or_path}' is the correct path to a directory containing a document_base.json file\n\n"  # noqa: E501
            )
            raise EnvironmentError(msg)

        if resolved_database_file == docbase_file:
            logger.info("loading database {}".format(docbase_file))
        else:
            logger.info("loading database {} from cache at {}".format(docbase_file, resolved_database_file))  # noqa: E501
